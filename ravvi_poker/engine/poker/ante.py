...
class Ante:
    ...
    def handle_last_round_type(self, last_round_type: Round):
        # обрабатывает результаты игры для того, чтобы определить следующее значение анте
        # если мы не на последнем уровне анте, то не имеет значение чем закончилась раздача (вскрытием или нет),
        # просто повышаем значение
        if self.current_ante_value != self.ante_levels[-1]:
            self.current_ante_value = self.ante_levels[self.ante_levels.index(self.current_ante_value)+1]
        # если на последнем, то смотрим чем закончилась раздача. Если вскрытием, то меняем значение анте на самое малое
        elif last_round_type is Round.SHOWDOWN:
