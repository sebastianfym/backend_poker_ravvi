...
class TablesManager(DBI_Listener):
    ...
    async def stop(self):
        ...
        async def wait_tables_closed():
            while self.tables:
                await asyncio.sleep(0.1)

        await wait_tables_closed()
    ...
    async def on_listen(self, db: DBI):
        ...
        for r in rows:
            await self.handle_table_start(r)
    ...
    async def on_table_profile_created(self, db: DBI, *, table_id):
        ...
        self.log.info("lock table id: %s ", table_id)
        await self.handle_table_start(row)

    async def on_table_profile_status(self, db: DBI, *, table_id, engine_status, engine_id):
    ...
    async def handle_table_start(self, row):
