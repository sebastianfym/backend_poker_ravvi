...
async def sample_scenario():
    ...
    async with client:
        ...
        # обновляем аккаунт
        await client.update_user_profile(name=f'USER-{client.user_id}', image_id=12)
        ...
        # Получить клуб по id
        await client.get_club_by_id(club_id=my_club[1].id)
        # Обновляем клуб
        await client.update_club(club_id=my_club[1].id, name=f"Club-{my_club[1].id}", description='my best club', image_id=12, timezone="Europe/Moscow", automatic_confirmation=True)
        # Получить список клубов для текущего пользователя
        await client.get_all_clubs()
        # Получить список участников клуба
        await client.get_clubs_members(my_club[1].id)
        # Отправить заявку на вступление в клуб
        await client.send_req_join_in_club(club_id=my_club[1].id, user_comment='Comment')
        # Создаем стол клуба
        await client.create_table(club_id=my_club[1].id, table_type="SNG", table_name="ClubTable", table_seats=9, game_type="NLH", game_subtype="REGULAR", buyin_cost=10)
        # Получение списка столов
        await client.get_club_tables(my_club[1].id)
        # Получение детального представления участника клуба для владельца
        await client.get_member_info_for_owner(my_club[1].id, client.user_id)
        # Получение своей анкеты участника в клубе
        await client.get_detail_member_info(my_club[1].id)
        # Получение баланса клуба
        await client.get_club_balance(my_club[1].id)
        # Получение списка запросов на пополнение баланса
        await client.get_club_chips_requests(my_club[1].id)
