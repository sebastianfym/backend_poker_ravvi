create or replace procedure create_txn_MOVEOUT(in operator_id bigint, in _club_id bigint, in _member_id bigint, in ref_member_id bigint, in txn_value numeric, out txn_id bigint) 
language plpgsql
as $body$
declare
    x_club_id bigint;
...
begin
	...
	insert into chips_txn
		...
		values 
        (_club_id, operator_id, 'MOVEOUT', txn_value, _member_id, ref_member_id) returning id into txn_id;
