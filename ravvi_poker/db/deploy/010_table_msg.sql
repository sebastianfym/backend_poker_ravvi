...
CREATE OR REPLACE FUNCTION table_msg_notify() RETURNS trigger 
AS $$
DECLARE
  payload VARCHAR;
  x VARCHAR;
BEGIN
  SELECT json_build_object('table_id',NEW.table_id,'msg_id', NEW.id)::VARCHAR INTO payload;
...
CREATE OR REPLACE 
TRIGGER table_msg_trigger
AFTER INSERT ON table_msg
FOR EACH ROW
EXECUTE FUNCTION table_msg_notify();