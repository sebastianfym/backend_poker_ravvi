create or replace procedure create_txn_BUYIN(in _member_id bigint, in _table_session_id bigint, in txn_value numeric, out txn_id bigint) 
language plpgsql
as $body$
declare
    _club_id bigint;
...
begin
    ...
    if balance_new<0 then 
        RAISE EXCEPTION '1003:Player balance % is too low for % txn', balance_old, -txn_value;
    end if;
	...
	insert into chips_txn
		...
		values 
        (_club_id, 0, 'BUYIN', txn_value, _member_id, _table_id) returning id into txn_id;
