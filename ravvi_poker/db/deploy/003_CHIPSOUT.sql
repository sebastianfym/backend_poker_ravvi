create or replace procedure create_txn_CHIPSOUT(in operator_id bigint, in _club_id bigint, in txn_value numeric, out txn_id bigint) 
language plpgsql
as $body$
declare
...
begin
	...
	insert into chips_txn 
		...
		values 
        (_club_id, operator_id, 'CHIPSOUT', txn_value) returning id into txn_id;
	-- create club record
