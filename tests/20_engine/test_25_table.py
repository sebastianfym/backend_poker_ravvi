...
async def test_user_lifecycle():
    ...
    async with db:
        await table.handle_cmd(db, cmd_id=11, user_id=USER_ID, client_id=100, cmd_type=Command.Type.JOIN,
                               props=dict(take_seat=False))
    ...
    async with db:
        await table.handle_cmd(db, cmd_id=12, user_id=USER_ID, client_id=100, cmd_type=Command.Type.TAKE_SEAT,
                               props=dict(seat_idx=3))
    # подтверждение
    ...
    table._user_enter_enabled = False
    async with db:
        await table.handle_cmd(db, cmd_id=13, user_id=USER_ID, client_id=101, cmd_type=Command.Type.JOIN,
                               props=dict(take_seat=True))
    # подтверждение подключения
    assert len(db._events) == 1
    assert db._event.table_id == TABLE_ID
    assert db._event.msg_type == Message.Type(TABLE_INFO_EVENT_TYPE)
    ...
    async with db:
        await table.handle_cmd(db, cmd_id=14, user_id=USER_ID, client_id=110, cmd_type=Command.Type.JOIN,
                               props=dict(take_seat=False))
    # подтверждение подключения
    assert len(db._events) == 1
    assert db._event.table_id == TABLE_ID
    ...
    # садимся на место 3
    async with db:
        await table.handle_cmd(db, cmd_id=15, user_id=USER_ID, client_id=100, cmd_type=Command.Type.TAKE_SEAT,
                               props=dict(seat_idx=3))
    # подтверждение
    assert len(db._events) == 1
    assert db._event.table_id == TABLE_ID
    ...
    async with db:
        await table.handle_cmd(db, cmd_id=16, user_id=123456, client_id=100, cmd_type=Command.Type.TAKE_SEAT,
                               props=dict(seat_idx=3))
    # подтверждение
    assert not db._events
    ...
    async with db:
        await table.handle_cmd(db, cmd_id=17, user_id=USER_ID, client_id=100, cmd_type=Command.Type.TAKE_SEAT,
                               props=dict(seat_idx=123456))
    # подтверждение
    assert not db._events

    # садимся на место 7
    async with db:
        await table.handle_cmd(db, cmd_id=18, user_id=USER_ID, client_id=100, cmd_type=Command.Type.TAKE_SEAT,
                               props=dict(seat_idx=7))
    # подтверждение
    assert len(db._events) == 1
    assert db._event.table_id == TABLE_ID
    ...
    table._user_enter_enabled = True
    async with db:
        await table.handle_cmd(db, cmd_id=22, user_id=USER_ID, client_id=111, cmd_type=Command.Type.JOIN,
                               props=dict(take_seat=True))
    # подтверждение подключения
    ...
    table._user_enter_enabled = False
    async with db:
        await table.handle_cmd(db, cmd_id=31, user_id=USER_ID, client_id=200, cmd_type=Command.Type.JOIN,
                               props=dict(take_seat=False))
    # подтверждение подключения
    assert len(db._events) == 1
    assert db._event.table_id == TABLE_ID
    ...
    table._user_enter_enabled = False
    async with db:
        await table.handle_cmd(db, cmd_id=32, user_id=USER_ID, client_id=201, cmd_type=Command.Type.JOIN,
                               props=dict(take_seat=True))
    # подтверждение подключения
    assert len(db._events) == 1
    assert db._event.table_id == TABLE_ID
    ...
    table._user_enter_enabled = True
    async with db:
        await table.handle_cmd(db, cmd_id=34, user_id=USER_ID, client_id=211, cmd_type=Command.Type.JOIN,
                               props=dict(take_seat=True))
    # подтверждение подключения
    assert len(db._events) == 1
    assert db._event.table_id == TABLE_ID
...
async def test_run_all_together():
    ...
    for i in range(1, 2):
        async with db:
            await table.handle_cmd(db, cmd_id=100 + i, user_id=i * 10, client_id=i * 100, cmd_type=Command.Type.JOIN,
                                   props=dict(take_seat=True))
        if i < 2:
            continue
    for i in range(2, 8):
        async with db:
            await table.handle_cmd(db, cmd_id=200 + i, user_id=i * 10, client_id=i * 100, cmd_type=Command.Type.JOIN,
                                   props=dict(take_seat=True))
        await stop_game()
    for i in range(60):
        async with table.lock:
