...
def test_message_private_info():
    users = [
        dict(user_id=10, cards_open=False, cards=[11, 12], hands=[
            dict(hands_belong="board1", hand_type='A', hand_cards=[11, 12, 13, 14, 15])
        ]),
        dict(user_id=20, cards_open=True, cards=[21, 22], hands=[
            dict(hands_belong="board1", hand_type='B', hand_cards=[21, 22, 23, 24, 25])
        ]),
        dict(user_id=30, cards_open=False, cards=[31, 32], hands=[
            dict(hands_belong="board1", hand_type='C', hand_cards=[31, 32, 33, 34, 35])
        ]),
    ]
    ...
    assert msg.users[0] == {'user_id': 10, 'cards': [11, 12],
                            'hands': [
                                {'hands_belong': 'board1', 'hand_type': 'A', 'hand_cards': [11, 12, 13, 14, 15]}
                            ],
                            "visible_cards": []}
    assert msg.users[1] == {'user_id': 20, 'cards': [21, 22],
                            'hands': [
                                {'hands_belong': 'board1', 'hand_type': 'B', 'hand_cards': [21, 22, 23, 24, 25]}
                            ]}
