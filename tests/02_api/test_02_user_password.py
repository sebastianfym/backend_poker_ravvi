...
def test_user_password():
    ...
    response = client.post("/v1/user/password", headers=headers, json=params)
    assert response.status_code == 200
...
def test_user_password_negative():
    # register new guest
    access_token = register_guest()
    headers = {"Authorization": "Bearer "+access_token}

    # try w/o access_token
    response = client.post("/v1/user/password", headers=None, json=None)
    assert response.status_code == 401

    # try no params
    response = client.post("/v1/user/password", headers=headers, json=None)
