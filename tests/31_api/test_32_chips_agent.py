...
def test_chips_txn_agent_smoke():
    owner = APITestClient()
    owner.register()
    owner.create_club()
    ...
    response = owner.post(f'/api/v1/clubs/{owner.club.id}/chips/agents/{agent.user_id}', json=params)
    assert response.status_code == 201
    ...
    agent.use_club()
    assert agent.club.user_balance == 0 
    assert agent.club.agents_balance == 200 
    ...
    agent.use_club()
    assert agent.club.user_balance == 0 
    assert agent.club.agents_balance == 100 
