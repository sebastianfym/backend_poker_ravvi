...
class DBI:
    ...
    async def create_player_account_txn(self, member_id, txntype, amount):
        async with self.cursor() as cursor:
            await cursor.execute("UPDATE user_account SET balance=balance+(%s) WHERE id=%s RETURNING balance",
                                 (amount, member_id))
            row = await cursor.fetchone()
        return row
    ...
    async def create_table(self, *, club_id=None, table_type, table_name, table_seats, game_type, game_subtype,
                           props=None):
